	...
	<div class="container containerScroll">
		...
		<?php while ($row = mysqli_fetch_array($query)): ?>
			...
			<div class="itemScroll col-md-6 col-md-offset-3 img-thumbnail" style="margin-bottom: 5px;" id="itemScroll-<?php echo $row['id']?>">
				...
				<button onclick="window.location.href='../DAO/validaCard.php?idPassado=<?=$row['id'];?>&valor=<?="naoValidar";?>'" class="btn btn-danger">NÃO VALIDAR</button>
			</div>
		<?php endwhile?>
		...
		<?php if (isset($next)): ?>
		<div class="navScroll">
			<a href='paraValidar.php?p=<?php echo $next?>'>Next</a>
		</div>
		<?php endif?>
	</div><!--.wrap-->
