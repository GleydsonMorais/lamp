	...
	<div class="container containerScroll">
		...
		<?php while ($row = mysqli_fetch_array($query)): ?>
			...
			<div class="itemScroll col-md-6 col-md-offset-3 img-thumbnail" style="margin-bottom: 5px; background: #00FF7F;" id="itemScroll-<?php echo $row['id']?>">
				...
				<button onclick="window.location.href='../DAO/validaCard.php?idPassado=<?=$row['id'];?>&valor=<?="naoValidar";?>'" class="btn btn-danger">DESVALIDAR</button>
			</div>
		<?php endwhile?>
		...
		<?php if (isset($next)): ?>
		<div class="navScroll">
			<a href='validados.php?p=<?php echo $next?>'>Next</a>
		</div>
		<?php endif?>
	</div><!--.wrap-->
